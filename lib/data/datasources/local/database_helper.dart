// lib/data/datasources/local/database_helper.dart (VERSÃO ATUALIZADA)

import 'dart:convert'; // ADICIONADO PARA O NOVO MÉTODO
import 'dart:io';
...
class DatabaseHelper {
  ...
  Future<int> insertProjeto(Projeto p) async =>
      await (await database).insert('projetos', p.toMap());
  Future<List<Projeto>> getTodosProjetos() async {
    final maps = await (await database)
        .query('projetos', orderBy: 'dataCriacao DESC');
  ...
  Future<void> deleteProjeto(int id) async =>
      await (await database).delete('projetos', where: 'id = ?', whereArgs: [id]);

  Future<int> insertAtividade(Atividade a) async =>
      await (await database).insert('atividades', a.toMap());
  Future<List<Atividade>> getAtividadesDoProjeto(int projetoId) async {
    final maps = await (await database).query('atividades',
        where: 'projetoId = ?',
        whereArgs: [projetoId],
  ...
  Future<void> deleteAtividade(int id) async =>
      await (await database).delete('atividades', where: 'id = ?', whereArgs: [id]);

  Future<void> insertFazenda(Fazenda f) async {
    await (await database)
        .insert('fazendas', f.toMap(), conflictAlgorithm: ConflictAlgorithm.fail);
  }

  Future<List<Fazenda>> getFazendasDaAtividade(int atividadeId) async {
    final maps = await (await database).query('fazendas',
        where: 'atividadeId = ?', whereArgs: [atividadeId], orderBy: 'nome');
    return List.generate(maps.length, (i) => Fazenda.fromMap(maps[i]));
  }

  Future<void> deleteFazenda(String id, int atividadeId) async {
    await (await database).delete('fazendas',
        where: 'id = ? AND atividadeId = ?', whereArgs: [id, atividadeId]);
  }

  Future<int> insertTalhao(Talhao t) async =>
      await (await database).insert('talhoes', t.toMap());

  Future<List<Talhao>> getTalhoesDaFazenda(
      String fazendaId, int fazendaAtividadeId) async {
    final db = await database;
  ...
  Future<double> getAreaTotalTalhoesDaFazenda(
      String fazendaId, int fazendaAtividadeId) async {
    final result = await (await database).rawQuery(
        'SELECT SUM(areaHa) as total FROM talhoes WHERE fazendaId = ? AND fazendaAtividadeId = ?',
        [fazendaId, fazendaAtividadeId]);
  ...
  Future<Parcela> saveParcela(Parcela parcela) async {
    final db = await database;
    final dbId = await db.insert('parcelas', parcela.toMap(),
  ...
  Future<List<Map<String, dynamic>>> getValoresCAP(int parcelaId) async {
    final db = await database;
    // Agora selecionamos tanto o 'cap' quanto o 'codigo'
    ...
    // Retorna a lista de mapas diretamente
    return result.map((row) => {
      'cap': row['cap'] as double,
  ...
  Future<Map<String, dynamic>> getDadosAgregadosDoTalhao(int talhaoId) async {
    ...
    // Compara enum com enum, que é o correto
  ...
  Future<List<Parcela>> getTodasAsParcelasConcluidas() async {
    final db = await database;
    // Aqui usamos o .name porque a cláusula `where` do SQL precisa de uma String
  ...
  Future<List<Talhao>> getTalhoesComParcelasConcluidas() async {
    ...
    // 1. Pega os IDs de todos os talhões que têm parcelas concluídas.
    ...
    // 2. Busca os dados completos desses talhões usando um JOIN.
    ...
    return List.generate(talhoesMaps.length, (i) {
      return Talhao.fromMap(talhoesMaps[i]);
    });
  }
  
  // --- MÉTODOS DE EXPORTAÇÃO ---
  Future<void> exportarDados(BuildContext context) async {
    final tipoExportacao = await showDialog<String>(context: context, builder: (c) => AlertDialog(title: const Text('Exportar Coletas de Parcela'), content: const Text('Selecione o tipo de exportação:'), actions: [TextButton(child: const Text('Novas Concluídas'), onPressed: () => Navigator.pop(c, 'novas_concluidas')), TextButton(child: const Text('Re-exportar Todas Concluídas'), onPressed: () => Navigator.pop(c, 'todas_concluidas')), TextButton(onPressed: () => Navigator.pop(c, 'cancelar'), child: const Text('Cancelar', style: TextStyle(color: Colors.grey)))]));
    ...
    try {
      ...
      if (tipoExportacao == 'novas_concluidas') {
        parcelasMaps = await db.query('parcelas', where: 'status = ? AND exportada = ?', whereArgs: [StatusParcela.concluida.name, 0]);
      } else {
        parcelasMaps = await db.query('parcelas', where: 'status = ?', whereArgs: [StatusParcela.concluida.name]);
      }
      if (parcelasMaps.isEmpty) {
        if (context.mounted) {
          ScaffoldMessenger.of(context).removeCurrentSnackBar();
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Nenhuma parcela encontrada para este critério.'), backgroundColor: Colors.orange));
        }
        return;
      }
      if (context.mounted) {
  ...
  Future<void> exportarCubagens(BuildContext context) async {
    final tipoExportacao = await showDialog<String>(context: context, builder: (c) => AlertDialog(title: const Text('Exportar Cubagens'), content: const Text('Selecione o tipo de exportação:'), actions: [TextButton(child: const Text('Exportar Novas'), onPressed: () => Navigator.pop(c, 'novas')), TextButton(child: const Text('Re-exportar Todas'), onPressed: () => Navigator.pop(c, 'todas')), TextButton(onPressed: () => Navigator.pop(c, 'cancelar'), child: const Text('Cancelar', style: TextStyle(color: Colors.grey)))]));
    if (tipoExportacao == null || tipoExportacao == 'cancelar' || !context.mounted) return;
    ...
    try {
      ...
      for (var a in arvores) {
        ...
        if (secoes.isEmpty) {
          rows.add([a.id, a.idFazenda, a.nomeFazenda, a.nomeTalhao, a.identificador, a.alturaTotal, a.valorCAP, null, null, null, null, null, null]);
        } else {
          for (var s in secoes) {
            rows.add([a.id, a.idFazenda, a.nomeFazenda, a.nomeTalhao, a.identificador, a.alturaTotal, a.valorCAP, s.alturaMedicao, s.circunferencia, s.casca1_mm, s.casca2_mm, s.diametroComCasca.toStringAsFixed(2), s.diametroSemCasca.toStringAsFixed(2)]);
          }
        }
      }
      final dir = await getApplicationDocumentsDirectory();
  ...
  Future<void> exportarUmaCubagem(BuildContext context, int arvoreId) async {
    ...
    if (arvoreMaps.isEmpty) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Árvore não encontrada.')));
      }
      return;
    }
    ...
    try {
      ...
      await db.transaction((txn) async {
        for (final feature in features) {
          ...
          // --- 1. Processar Projeto ---
          Projeto? projeto = await txn
              .query('projetos',
                  where: 'nome = ?', whereArgs: [properties['projeto_nome']])
              .then((list) =>
                  list.isEmpty ? null : Projeto.fromMap(list.first));
          if (projeto == null) {
            projeto = Projeto(
              nome: properties['projeto_nome'],
              empresa: properties['projeto_empresa'],
              responsavel: properties['projeto_responsavel'],
          ...
          // --- 2. Processar Atividade ---
          Atividade? atividade = await txn
              .query('atividades',
                  where: 'tipo = ? AND projetoId = ?',
                  whereArgs: [properties['atividade_tipo'], projeto.id])
              .then((list) =>
                  list.isEmpty ? null : Atividade.fromMap(list.first));
          if (atividade == null) {
            atividade = Atividade(
              projetoId: projeto.id!,
              tipo: properties['atividade_tipo'],
              descricao: properties['atividade_descricao'] ?? '',
              dataCriacao: DateTime.now(),
            );
            final atividadeId =
          ...
          // --- 3. Processar Fazenda ---
          Fazenda? fazenda = await txn
              .query('fazendas',
                  where: 'id = ? AND atividadeId = ?',
                  whereArgs: [properties['fazenda_id'], atividade.id])
              .then((list) =>
                  list.isEmpty ? null : Fazenda.fromMap(list.first));
          if (fazenda == null) {
            fazenda = Fazenda(
              id: properties['fazenda_id'],
              atividadeId: atividade.id!,
              nome: properties['fazenda_nome'],
              municipio: properties['fazenda_municipio'],
              estado: properties['fazenda_estado'],
            );
          ...
          // --- 4. Processar Talhão ---
          Talhao? talhao = await txn
              .query('talhoes',
                  where: 'nome = ? AND fazendaId = ? AND fazendaAtividadeId = ?',
                  whereArgs: [
                properties['talhao_nome'],
                fazenda.id,
                fazenda.atividadeId
              ])
              .then(
                  (list) => list.isEmpty ? null : Talhao.fromMap(list.first));
          if (talhao == null) {
            talhao = Talhao(
          ...
          // --- 5. Processar Parcela ---
          Parcela? parcela = await txn
              .query('parcelas',
                  where: 'idParcela = ? AND talhaoId = ?',
                  whereArgs: [properties['parcela_id_plano'], talhao.id])
              .then((list) =>
                  list.isEmpty ? null : Parcela.fromMap(list.first));
          if (parcela == null) {
            ...
            parcela = Parcela(
              ...
              status: StatusParcela.pendente, // Sempre importa como pendente
