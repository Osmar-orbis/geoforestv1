...
class DatabaseHelper {
  ...
  Future<void> exportarDados(BuildContext context) async {
    ...
    if (tipoExportacao == 'nao_exportadas') {
      parcelasMaps = await db.query('parcelas', where: 'exportada = 0');
    } else {
    ...
    try {
      ...
      List<List<dynamic>> rows = [['ID_Db_Parcela', 'Fazenda', 'Talhao', 'ID_Coleta_Parcela', 'Area_m2', 'Espacamento', 'Observacao_Parcela', 'Easting', 'Northing', 'Data_Coleta', 'Status_Parcela', 'Linha', 'Posicao_na_Linha', 'Fuste_Num', 'Status_Arvore', 'Status_Arvore_2', 'CAP_cm', 'Altura_m', 'Dominante']];
      ...
      for (var parcelaMap in parcelasMaps) {
        ...
        if (parcelaMap['latitude'] != null && parcelaMap['longitude'] != null) {
          var pontoOrigem = proj4.Point(x: parcelaMap['longitude'] as double, y: parcelaMap['latitude'] as double);
          var pontoUtm = projWGS84.transform(projUTM, pontoOrigem);
        ...
        if (arvores.isEmpty) {
          rows.add([parcelaMap['id'], parcelaMap['nomeFazenda'], parcelaMap['nomeTalhao'], parcelaMap['idParcela'], parcelaMap['areaMetrosQuadrados'], parcelaMap['espacamento'], parcelaMap['observacao'], easting, northing, parcelaMap['dataColeta'], parcelaMap['status'], null, null, null, null, null, null, null, null]);
        } else {
          ...
          for (final arvore in arvores) {
            ...
            rows.add([
                parcelaMap['id'], parcelaMap['nomeFazenda'], parcelaMap['nomeTalhao'], parcelaMap['idParcela'],
                parcelaMap['areaMetrosQuadrados'], parcelaMap['espacamento'], parcelaMap['observacao'],
                easting, northing, parcelaMap['dataColeta'], parcelaMap['status'],
                arvore.linha, arvore.posicaoNaLinha, fusteCounter[posKey],
                arvore.status.name, arvore.status2?.name, arvore.cap, arvore.altura, arvore.dominante ? 'Sim' : 'Não'
            ]);
          }
        }
      }
