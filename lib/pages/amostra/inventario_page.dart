...
class _InventarioPageState extends State<InventarioPage> {
  ...
  Widget build(BuildContext context) {
    final listaExibida = _mostrandoApenasDominantes ? _arvoresColetadas.where((a) => a.dominante).toList() : _arvoresColetadas;
    return Scaffold(
      appBar: AppBar(
        title: const Text('Coleta da Parcela'),
        actions: [
          if (_isSaving) const Padding(padding: EdgeInsets.only(right: 16.0), child: Center(child: SizedBox(width: 20, height: 20, child: CircularProgressIndicator(color: Colors.white))))
          else ...[
